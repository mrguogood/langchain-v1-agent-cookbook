...
# # 4、ANN搜索
# # 近似最近邻（ANN）搜索以记录向量嵌入排序顺序的索引文件为基础
# # 根据接收到的搜索请求中携带的查询向量通过索引文件快速定位一个可能包含与查询向量最相似的向量嵌入的子组（子集），
# 指定的度量类型来衡量查询向量与该子组中向量之间的相似度，根据与查询向量的相似度对组内成员进行排序，并确定排名前 K 的组内成员
# # 定义要搜索的问题文本
...
# # 5、使用标准过滤再进行ANN搜索
# # 若集合中同时包含向量嵌入（密集向量，content_dense）及其元数据（pubAuthor等），可以在 ANN 搜索之前过滤元数据（sql中的where条件），以提高搜索结果的相关性
