...
class MilvusCollectionManager:
    ...
    def create_schema(self) -> Optional[Any]:
        ...
        try:
            ...
            # 分词结果: ["AI", "浏览器"]
            # ↓  BM25 函数
            # 稀疏向量: {1234: 0.8, 5678: 1.2}   ← 只存储有意义的词及其权重
            # ↓  自动写入
            # title_sparse 字段
            # 这个过程是 自动的 — Milvus 的内置 BM25 函数会调用你配置的分词器，从原始文本字段（如 title）生成稀疏向量（如 title_sparse）
            # 搜索时，Milvus 对 data 中的查询文本使用相同的分词器分词，然后在稀疏向量中做匹配。
            # 写入数据:  原始文本 → [分词器] → Token列表 → [BM25函数] → 稀疏向量 → 存入Milvus
            # 查询数据:  查询文本 → [相同分词器] → Token列表 → [BM25匹配] → 找到相关文档
            schema.add_field(
                field_name="title",
                datatype=DataType.VARCHAR,
                max_length=1000,
                analyzer_params=analyzer_params,#绑定分词器
            ...
            # 添加content_dense字段，存储文档块的密集向量嵌入（1024维）
            schema.add_field(
