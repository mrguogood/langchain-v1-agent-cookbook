...
|        LLM        |                             介绍                             |
| :---------------: | :----------------------------------------------------------: |
|      OpenAI       |            直连 OpenAI 官方 API，调用 GPT-4等模型            |
|      OneAPI       | 一个中间代理，统一管理多个模型厂商的入口，你只需对接 OneAPI，它帮你转发到真正的模型 |
| 阿里云百炼 (qwen) |   直连阿里云 DashScope 平台，调用通义千问（qwen）系列模型    |
|      Ollama       | 在你的本地电脑上跑开源模型（如 Llama3），不依赖任何云服务，数据不出本机 |
