...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--playwright",
        action="store_true",
        help="使用内置问题：用浏览器工具打开 example.com 并概括页面用途（需 Playwright 与 Chromium 已安装）",
    )
    # 解析命令行参数
    ...
    if args.playwright:
        args.question = (
            "请用浏览器工具打开 https://juejin.cn/ ，"
            "用 extract_text 或 get_elements 获取页面主要可见文本，并简要说明该页面用途。"
        )

    # 打印测试开始信息
    print("开始测试 ReAct Agent API（含 Skill + Playwright，14）...\n")
    # 打印后端地址、用户 ID、会话 ID
