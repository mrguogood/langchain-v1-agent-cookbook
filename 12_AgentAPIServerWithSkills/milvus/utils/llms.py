...
MODEL_CONFIGS = {
    ...
    "qwen": {
        ...
        # DashScope 的 API Key
        "api_key": "sk-3f59f1b1bacc43c094e27742862fee70",
        # 对话模型名称
        "chat_model": "qwen-max",
        # 向量嵌入模型名称
        "embedding_model": "text-embedding-v3"
...
def initialize_llm(llm_type: str = DEFAULT_LLM_TYPE) -> tuple[ChatOpenAI, OpenAIEmbeddings | DashScopeEmbeddings]:
    """
    初始化LLM实例
    ...
    try:
        ...
        # 创建向量嵌入模型实例
        if llm_type == "qwen":
            # qwen 使用 DashScope 原生 SDK，避免 OpenAI 兼容端点 embeddings 的 bug
...
def get_llm(llm_type: str = DEFAULT_LLM_TYPE) -> tuple[ChatOpenAI, OpenAIEmbeddings | DashScopeEmbeddings]:
    """
...
if __name__ == "__main__":
    try:
        # 测试使用 qwen 配置初始化 LLM 与 Embedding
