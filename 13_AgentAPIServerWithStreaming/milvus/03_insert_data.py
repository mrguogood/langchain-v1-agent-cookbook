        ...
        try:
            ...
            for doc_idx, doc_data in enumerate(tqdm(documents, desc="处理文档")):
                ...
                try:
                    ...
                    # 记录文档的块数量到统计字典
                    logger.debug(f"文档 {doc_data['docId']}，分割后数据为：{content_chunks}，已分割为 {len(content_chunks)} 个块")
                    ...
                    for chunk_idx, chunk in enumerate(content_chunks):
                        ...
                        try:
                            ...
                            logger.debug(f"文档 {doc_data['docId']} 已生成{chunk_idx}块向量，块向量为：{dense_vector}")
                            # 构造文档块数据字典
                            chunk_data = {
                    ...
                    # 记录文档分割完成的调试日志，包含块数量
                    logger.debug(f"文档 {doc_data['docId']} 已处理 {len(content_chunks)} 个块数据")
